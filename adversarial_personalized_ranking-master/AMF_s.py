...
def parse_args():
    ...
    parser.add_argument('--embed_size', type=int, default=64,    # 模型矩阵的隐层维
                        help='Embedding size.')
    parser.add_argument('--dns', type=int, default=1,
                        help='number of negative sample for each positive in dns.') # dns每个训练样本中对抗样本的个数
    parser.add_argument('--reg', type=float, default=0,
                        help='Regularization for user and item embeddings.') # 用户和项目嵌入的正规化
    parser.add_argument('--lr', type=float, default=0.05,                       # 学习率
                        help='Learning rate.')
    parser.add_argument('--reg_adv', type=float, default=1,
                        help='Regularization for adversarial loss')  # 对抗性损失的正规化
    parser.add_argument('--restore', type=str, default=None,
                        help='The restore time_stamp for weights in \Pretrain') # \Pretrain中权重的恢复时间戳
    parser.add_argument('--ckpt', type=int, default=100,
                        help='Save the model per X epochs.')   # 每X个世代保存模型。
    parser.add_argument('--task', nargs='?', default='',
                        help='Add the task name for launching experiments')   # 添加启动实验的任务名称
    ...
    parser.add_argument('--adv', nargs='?', default='grad',
                        help='Generate the adversarial sample by gradient method or random method')  # 通过梯度法或随机方法生成对抗样本
    parser.add_argument('--eps', type=float, default=0.5,
                        help='Epsilon for adversarial weights.')  # eps即ϵ
...
# 返回
def shuffle(samples, batch_size, dataset, model,args):  # shuffle：拖曳
    global _user_input
    ...
    global _model
    global _dataset
    _user_input, _item_input_pos = samples  # 表示所有评价的用户和物品
    _batch_size = batch_size                # 单个训练集大小
    _index = list(range(len(_user_input)))# 修改3 _index:评价个数
    _model = model                          # 模型
    _dataset = dataset                      # 数据集类
    np.random.shuffle(_index) # np.random.shuffle(x) 现场修改序列，改变自身内容。（类似洗牌，打乱顺序）
    num_batch = len(_user_input) // _batch_size
    pool = Pool(cpu_count())
    # res为3维空间[[[],[],[],[]],[[],[],[],[]]]
    res = pool.map(partial(_get_train_batch,_batch_size=_batch_size,_index=_index,_user_input=_user_input,_item_input_pos=_item_input_pos,_dataset=_dataset,_args=args), range(num_batch)) # 修改3
    pool.close()
    pool.join()
    user_list = [r[0] for r in res] # 二维空间[[],[]]
    item_pos_list = [r[1] for r in res] # 二维空间[[],[]]
    user_dns_list = [r[2] for r in res] # 二维空间[[],[]]
    item_dns_list = [r[3] for r in res] # 二维空间[[],[]]
    return user_list, item_pos_list, user_dns_list, item_dns_list


def _get_train_batch(i,_batch_size,_index,_user_input,_item_input_pos,_dataset,_args): # 承接修改3
    ...
    # args = parse_args()
    # # args.adver = 0
    _model = MF(_dataset.num_users, _dataset.num_items, _args)
    _model.build_graph()
    for idx in range(begin, begin + _batch_size):   # 修改4
    # for idx in range(0, begin):
    #     Artifact = _model.dns  # 修改5：神器
        user_batch.append(_user_input[_index[idx]])
        item_batch.append(_item_input_pos[_index[idx]])
        for dns in range(_model.dns): # dns用户u未评论的物品个数
    ...
    return np.array(user_batch)[:, None], np.array(item_batch)[:, None], \
           np.array(user_neg_batch)[:, None], np.array(item_neg_batch)[:, None] # 由一维矩阵--二维矩阵（shape（n,1）)
...
class MF:
    ...
    def _create_variables(self):
        with tf.name_scope("embedding"):
            ...
            self.delta_P = tf.Variable(tf.zeros(shape=[self.num_users, self.embedding_size]),
                                       name='delta_P', dtype=tf.float32, trainable=False)  # (users, embedding_size) delta矩阵
            self.delta_Q = tf.Variable(tf.zeros(shape=[self.num_items, self.embedding_size]),
                                       name='delta_Q', dtype=tf.float32, trainable=False)  # (items, embedding_size) delta矩阵
    ...
    def _create_inference(self, item_input):
        with tf.name_scope("inference"):
            ...
            return tf.matmul(self.embedding_p * self.embedding_q,               # 先计算加减，故设计成此种运算形式，还有另一种tf.reduce_sum(tf.multiply(u,(i-j))
    ...
    def _create_loss(self):
        with tf.name_scope("loss"):
            ...
            self.result = tf.clip_by_value(self.output - self.output_neg, -80.0, 1e8) # tf.clip_by_value(A, min, max)：输入一个张量A，
            ...
            self.loss = tf.reduce_sum(tf.nn.softplus(-self.result)) # softplus函数：log(1+exp(x)),平滑版的relu函数,

            # loss to be omptimized
            self.opt_loss = self.loss + self.reg * tf.reduce_mean(
                tf.square(embed_p_pos) + tf.square(embed_q_pos) + tf.square(embed_q_neg))  # embed_p_pos == embed_q_neg tf.square：对张量的每个值求平方
    ...
    def _create_adversarial(self):
        with tf.name_scope("adversarial"):
            ...
            if self.adv == "random":
                ...
                self.update_P = self.delta_P.assign(tf.nn.l2_normalize(self.adv_P, 1) * self.eps)   # 公式（8）
            ...
            # generate the adversarial weights by gradient-based method
                ...
                # convert the Indexed Slice Data to Dense Tensor 将索引切片数据转换为密集张量
...
def training(model, dataset, args, epoch_start, epoch_end, time_stamp) :
    # saver is an object to save pq
    with tf.Session() as sess:
        writer = tf.summary.FileWriter("E://TensorBoard//log", sess.graph)  # 写入tensorboard
        ...
        if ckpt_restore_path and not os.path.exists(ckpt_restore_path):# 如果字串非0，对应的目录不存在，则创建目录
        ...
        eval_feed_dicts = init_eval_model(model, dataset)  # 2019-4-25分析到此
        ...
        for epoch_count in range(epoch_start, epoch_end + 1):
            ...
            batch_begin = time()
            batches = shuffle(samples, args.batch_size, dataset, model,args)
            ...
            if model.epochs == epoch_count:
                ...
                for idx, (hr_k, ndcg_k, auc_k) in enumerate(np.swapaxes(best_res['result'], 0, 1)):
                    ...
                    with open("Save/NGCD100-small.txt", "a") as f:
                        f.write(res + '\n')
                    print(res)
...
def output_evaluate(model, sess, dataset, train_batches, eval_feed_dicts, epoch_count, batch_time, train_time, prev_acc,
    ...
    hr, ndcg, auc = np.swapaxes(result, 0, 1)[-1]  # K = 100时，hr,ndcg,auc的值
    ...
    with open("Save/APR_BPR_0_2000-small.txt", "a") as f:
        f.write(res + '\n')
    print(res)
...
def training_batch(model, sess, batches, adver=False):
    ...
    if model.dns == 1:
        ...
        for i in range(len(user_input)):
            feed_dict = {model.user_input: user_input[i], # 512条数据
                         model.item_input_pos: item_input_pos[i], # 512条数据
                         model.item_input_neg: item_input_neg[i]} # 512条数据
            if adver:
                sess.run([model.update_P, model.update_Q], feed_dict) # 需要理解图方可理解，疑惑点1
    ...
    # dns > 1, i.e., BPR-dns
        ...
        for i in range(len(user_input)):
            ...
            output_neg = sess.run(model.output_neg, feed_dict) # 需要理解图，疑惑点2
...
def init_eval_model(model, dataset):
    ...
    _model = model
    # print("---")
    # print(_dataset.testRatings[_dataset.num_users-1][1])
    pool = Pool(cpu_count())
    feed_dicts = pool.map(partial(_evaluate_input,_dataset=dataset), range(_dataset.num_users)) # 修改1
    pool.close()
    pool.join()
...
def _evaluate_input(user,_dataset):
    # generate items_list

    test_item = _dataset.testRatings[user][1] # 前user个数据的item

    item_input = set(range(_dataset.num_items)) - set(_dataset.trainList[user])# 前user个user评价的所有item集合
    if test_item in item_input:
        item_input.remove(test_item)
    item_input = list(item_input)
    item_input.append(test_item)
    user_input = np.full(len(item_input), user, dtype='int32')[:, None]# np.full自动生成值为arg2的行向量
    item_input = np.array(item_input)[:, None]# 竖着切片，生成增加一维的列向量
    return user_input, item_input  # user_input:[user,user,user...],item_input[user对应的]
...
def evaluate(model, sess, dataset, feed_dicts, output_adv):
    ...
    res = []
    # for user in range(_dataset.num_users): # 修改
    for user in range(_dataset.num_users):   # 最高5000，与上面一个函数一致，不然报列表越界
...
def _eval_by_user(user):
    ...
    position = (neg_predict >= pos_predict).sum()         # 只有数组对象才可以(neg_predict >= pos_predict)这样用，sum()方法返回neg_predict中符合条件的个数

    # calculate from HR@1 to HR@100, and from NDCG@1 to NDCG@100, AUC
    hr, ndcg, auc = [], [], []
    for k in range(1, _K + 1):
        hr.append(position < k) # 真:True扔进hr，否则仍False
